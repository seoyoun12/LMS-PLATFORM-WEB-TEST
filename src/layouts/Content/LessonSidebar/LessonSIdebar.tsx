...
export function LessonSidebar({course,tabsConfig, testList , noticeConfig ,fileList}:Props){
    ...
    return <StickySideBar>
      <TabMenu tabsConfig={tabsConfig} showBorderBottom={false} onChangeMenu={onChangeMenu} rerender={false} sortFull={true} changeMenu={changeMenu} /> {/*메뉴바 헤더 */}
      {/*첫 메뉴 리스트 */}
    ...
    <TabPanel value={changeMenu} index={tabsConfig[1].value} >
        <MenuList>
          {noticeConfig.map((notice,index)=>{
            return <>
            <MenuCellItem key={index} onClick={()=>{
              setRevealVideo(false)
              router.push({
                pathname,
                query:{
              ...
              <Box>
                ...
                <LessonInfo>
                  <Typography className="typo" variant="body2" color={grey[500]}>{dateFormat(notice.date,"yyyy.mm.dd")}</Typography>
                </LessonInfo>
              </Box>
              <LessonCheck >
            ...
            {index === (noticeConfig.length - 1) ? "" : <Divider sx={{borderBottom:`2px solid ${grey[500]} `}} />} {/*마지막이면  디비더 없음. */}
            </>
          })}
      </MenuList>
    ...
    <TabPanel value={changeMenu} index={tabsConfig[2].value} >
      <MenuList>
      <Typography className='file-list-title' variant='h6' >수업자료</Typography>
        {fileList.map((file , index)=>{
          return <MenuCellItem className='file-list-item' key={index} color={grey[900]} >
                  <Box>{file.title}</Box>
                  <LessonCheck>
                      <DownloadIcon />
                  </LessonCheck>

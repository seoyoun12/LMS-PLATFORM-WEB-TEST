    ...
    <CreateUserwrap>
      <Container>
        <Box component="form" onSubmit={handleSubmit(createSubmit)}>
          ...
          <Typography sx={{ color: 'red' }}>운수,저상으로 생성하는 경우 아이디, 패스워드는 주민등록번호로 해야합니다!</Typography>
          ...
          <TextField
            type={watch().regCategory === regCategoryType.TYPE_TRANS_EDU ? 'password' : 'text'}
            {...register('username')}
            fullWidth
          />
          ...
          <TextField
            type="password"
            {...register('password', { pattern: { value: passwordRegex, message: 'Invailed password' } })}
            fullWidth
          />
          <ErrorMessage errors={errors} name="password" render={({ message }) => <Box color="red">{message}</Box>} />
          <Typography>핸드폰('-' 없이 작성)</Typography>
          <TextField {...register('phone', { pattern: { value: phoneRegex, message: 'Invailed Phone' } })} fullWidth />
          <ErrorMessage errors={errors} name="phone" render={({ message }) => <Box color="red">{message}</Box>} />
          <Typography>smsYn</Typography>
          <Checkbox onChange={(e, checked) => (checked ? setValue('smsYn', YN.YES) : setValue('smsYn', YN.YES))} /> <span>SMS동의</span>
          <Typography>email</Typography>
          <TextField {...register('email', { pattern: { value: emailRegex, message: 'Invailed Email' } })} fullWidth />
          <ErrorMessage errors={errors} name="email" render={({ message }) => <Box color="red">{message}</Box>} />
          <Typography>emailYn</Typography>
          <Checkbox onChange={(e, checked) => (checked ? setValue('emailYn', YN.YES) : setValue('emailYn', YN.NO))} />{' '}
          <span>이메일동의</span>

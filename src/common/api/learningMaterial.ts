...
// [관리자] 학습자료 전체 조회
export function learningMaterialList({ page, elementCnt, materialType, materialSubType
} : {
  page: number;
  elementCnt?: number;
  materialType?: string;
  ...
  const { data, error, mutate } = useSWR<SWRResponse<PaginationResult<LearningMaterial[]>>>([
...
export function learningMaterialDetail(seq: number | null) {
  const { data, error, mutate }
   = useSWR<SWRResponse<LearningMaterial>>(seq? `/learning-material/adm/${seq}` : null, GET);
  return {
    data: data?.data,
    error,
    mutate
...
// [관리자] 학습 자료 생성
export async function learningMaterialUpload(learningMaterialInput : LearningMaterialInput) {
  return await POST(`/learning-material/adm`, learningMaterialInput)
}


// [관리자] 학습 자료 수정
export async function learningMaterialModify({seq, learningMaterialInput} : {
  seq: number, learningMaterialInput: LearningMaterialInput;
}) {
